...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: buildAlternates({ pathSuffix: `/creators/country/${country}` }),
    openGraph: {
      title, description: meta.description,
      url: `${publicConfig.siteUrl}/creators/country/${country}`,
...
export default async function CountryPage({
  ...
  const breadcrumbLd = buildBreadcrumbListLd([
    ...
    { name: meta.label, url: `${base}/creators/country/${country}` },
  ]);
  const itemListLd = buildItemListLd(
    creators.map((c) => ({ name: c.name, url: `${base}/creator/${c.slug}`, description: c.description })),
  );
