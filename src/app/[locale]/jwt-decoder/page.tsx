...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: { type: "website", title: "JWT Decoder – Decode JWT Tokens Locally", description: "Decode JWT tokens locally. View claims and expiry.", url: `${publicConfig.siteUrl}/${locale}${PATH}`, siteName: publicConfig.siteName, locale },
...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      ...
      <ToolSEO locale={locale} pathSuffix={PATH} toolName="JWT Decoder" toolDescription="Browser-based JWT token decoder with claim inspection and expiry checking." faq={FAQ} breadcrumbName="JWT Decoder" />
    </>
  );
}
