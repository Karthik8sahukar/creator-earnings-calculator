...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: buildAlternates({ locale, pathSuffix: `/top-creators/${filter}` }),
    openGraph: {
      title: data.title,
      description: data.description,
      url: `${publicConfig.siteUrl}/${locale}/top-creators/${filter}`,
...
export default async function FilteredRankingsPage({
  ...
  const breadcrumbLd = buildBreadcrumbListLd([
    ...
    { name: data.title, url: `${publicConfig.siteUrl}/${locale}/top-creators/${filter}` },
  ]);
