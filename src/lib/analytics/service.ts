...
import { getAnalyticsStorageSafe } from "./storage";
 ...
 *
 * Never throws — returns empty analytics when no data exists or
...
export async function getCreatorAnalytics(
  ...
  // Storage unavailable — return empty analytics gracefully
  if (!storage) {
    return {
      creatorSlug,
      snapshots: [],
      growth: calculateAllGrowth([]),
      lastUpdated: null,
    };
  }
