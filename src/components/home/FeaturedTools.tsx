...
const TOOLS: readonly Tool[] = [
  ...
  },
  {
    ...
    badgeKey: "featuredTools.badges.streaming",
    badgeColor: BADGE_STREAMING,
    Icon: DollarIcon,
  },
];
 ...
 * YouTube Money Calculator gets a featured (larger) card.
 * No decision/utility tools here (those are in DecisionTools section).
 */
  ...
  return (
    <section id="tools" aria-labelledby="featured-tools-title" className="scroll-mt-20">
      ...
      <ul className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {TOOLS.map(({ href, titleKey, descriptionKey, badgeKey, badgeColor, Icon, featured }) => (
          <li key={href} className={featured ? "sm:col-span-2 lg:col-span-1" : ""}>
            <Link
              href={href as "/youtube-rpm-calculator"}
              className={`group card flex h-full flex-col gap-4 transition duration-200 hover:-translate-y-1 hover:shadow-pop focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/60 ${
                featured
                  ? "p-6 sm:p-7 border-2 border-brand-200 dark:border-brand-700/50"
                  : "p-5"
              }`}
            >
              <div className="flex items-start justify-between">
                <span className={`inline-flex items-center justify-center rounded-xl bg-gradient-to-br from-brand-500/15 to-accent-500/15 text-brand-700 dark:text-brand-200 ${featured ? "h-12 w-12" : "h-10 w-10"}`}>
                  <Icon width={featured ? 22 : 18} height={featured ? 22 : 18} />
                </span>
              ...
              <div className="flex-1 space-y-2">
                <h3 className={`font-semibold text-slate-900 dark:text-slate-100 ${featured ? "text-lg" : "text-base"}`}>
                  {t(titleKey)}
                  {featured && (
                    <span className="ml-2 inline-flex items-center rounded bg-brand-100 px-1.5 py-0.5 text-[9px] font-bold uppercase text-brand-700 dark:bg-brand-500/20 dark:text-brand-200">
                      Featured
                    </span>
                  )}
                </h3>
