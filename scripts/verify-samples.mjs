...
console.log("Country  | Long-form baseRpm exp | Shorts baseRpm exp");
for (const id of ["US", "GB", "DE", "IN", "BR", "OTHER"]) {
  const c = findCountry(id);
  console.log(`${c.label.padEnd(20)} | $${c.baseRpm.expected.toString().padEnd(4)} | $${c.shortsRpm.expected}`);
}
console.log("");
console.log("Niche         | Long-form mult | Shorts mult");
for (const id of ["finance", "business", "tech", "education", "other", "entertainment", "gaming", "music", "kids"]) {
  const n = findNiche(id);
  console.log(`${n.label.padEnd(35)} | ${n.rpmMultiplier.toFixed(2).padEnd(6)} | ${n.shortsRpmMultiplier.toFixed(2)}`);
}
console.log("");
...
console.log("");
console.log("=== Conservative → Optimistic band spread (US Tech Long-form, 1M views) ===");
const uTech = calculateEarnings({
...
console.log(`Conservative: ${money(uTech.low.monthly)}`);
console.log(`Expected:     ${money(uTech.expected.monthly)}`);
console.log(`Optimistic:   ${money(uTech.high.monthly)}`);
